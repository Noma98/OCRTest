import { AppState } from 'react-native';
import {
  Camera,
  DataCaptureContext,
  FrameSourceState,
  CameraPosition,
} from 'scandit-react-native-datacapture-core';
import { IdCapture } from 'scandit-react-native-datacapture-id';

import { isAndroid } from '@/utils/check';

export default class ContextSingleton {
  static instance = ContextSingleton.instance || new ContextSingleton();

  constructor() {
    // hook for handling the camera singleton out of the scanning page
    AppState.addEventListener('change', this.handleAppStateChange);
  }

  getRearCamera() {
    if (!this.rearCamera) {
      this.rearCamera = Camera.default;
      this.rearCamera.applySettings(IdCapture.recommendedCameraSettings);
    }
    return this.rearCamera;
  }

  getFrontCamera() {
    if (!this.frontCamera) {
      this.frontCamera = Camera.atPosition(CameraPosition.UserFacing);
      this.frontCamera.applySettings(IdCapture.recommendedCameraSettings);
    }
    return this.frontCamera;
  }

  handleAppStateChange = async nextAppState => {
    // turning off the camera if the app goes to background
    if (nextAppState.match(/inactive|background/) && this.camera) {
      console.log('Singleton inactive hook');
      this.rearCamera.switchToDesiredState(FrameSourceState.Off);
      this.frontCamera.switchToDesiredState(FrameSourceState.Off);
    }
  };

  getDataCaptureContext() {
    if (!this.dataCaptureContext) {
      this.dataCaptureContext = DataCaptureContext.forLicenseKey(
        isAndroid()
          ? ''
          : '',
      );
    }
    return this.dataCaptureContext;
  }
}
